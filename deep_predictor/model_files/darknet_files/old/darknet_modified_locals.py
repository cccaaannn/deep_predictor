...
if os.name == "nt":
    ...
    lib = CDLL("deep_predictor/model_files/darknet_files/libdarknet.so", RTLD_GLOBAL)
...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45, debug= False, altNames=None):
    """
    ...
    ret = detect_image(net, meta, im, thresh, hier_thresh, nms, debug,altNames)
    w, h = im.w, im.h
    free_image(im)
...
def performDetect(imagePath="data/dog.jpg", thresh= 0.25, configPath = "./cfg/yolov4.cfg", weightPath = "yolov4.weights", metaPath= "./cfg/coco.data", showImage= True, makeImageOnly = False, initOnly= False):
    ...
    detections, w, h = detect(netMain, metaMain, imagePath.encode("ascii"), thresh, altNames=altNames)
    if showImage:
        try:
